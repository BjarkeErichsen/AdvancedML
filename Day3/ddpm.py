...
class DDPM(nn.Module):
    ...
    def negative_elbo(self, x):
        ...
        """
        T = self.T
        t = torch.randint(low = 1, high = T+1)
        epsilon = torch.normal(torch.zeros(x.shape), torch.ones(x.shape))
        zt = torch.sqrt(self.alpha_cumprod[t]) * x + torch.sqrt(1 - self.alpha_cumprod[t])*epsilon
        pred_eps = self.network(zt, t)
        neg_elbo = F.mse_loss(epsilon - pred_eps)
        """
        T = self.T
        t = torch.randint(low=1, high=T, size=(x.shape[0],1))
    ...
    def sample(self, shape):
        ...
        for t in range(self.T-1, -1, -1):
            coefficient = (1-torch.sqrt(self.alpha[t])) / (torch.sqrt(1-torch.sqrt(self.alpha_cumprod[t])))
            ### Implement the remaining of Algorithm 2 here ###

            if t > 1:
                z = torch.normal(torch.zeros(shape), torch.ones(shape))
                sigma_t = torch.sqrt( (1-self.alpha_cumprod[t-1]) / (1- self.alpha_cumprod[t]) * self.beta[t])
                sigma_and_z = sigma_t * z
            else:
                z = torch.zeros(shape)
                sigma_and_z = z

            x_t = 1/torch.sqrt(self.alpha[t]) * (x_t - coefficient*self.network(x_t, t)) + sigma_and_z
        return x_t
